...
export default function Products({ allProducts }) {
  ...
  const filteredProducts = useMemo(() => {
    if (searchQuery === '') {
      ...
      const filtered = allProducts.filter((product) => product.title.toLowerCase().includes(lowerCaseQuery))
      return filtered
    }
                      ...
                      <div className="mt-1.5 flex flex-col items-center justify-between text-text">
                        ...
                        <div className="col-span-12 text-center w-full mt-3">
                          <button
                            ...
                            className="disabled block rounded bg-primary px-5 py-3 text-md text-text w-full transition hover:bg-secondary hover:text-white"
                          >
                            Add to cart
                          </button>
                        </div>
